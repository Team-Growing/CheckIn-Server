...
public interface LectureRepository extends JpaRepository<LectureJpaEntity, Long> {
    ...
    @Query("SELECT l FROM LectureJpaEntity l LEFT JOIN FETCH l.dayOfWeekVO d WHERE l.lectureStatus = :lectureStatus")
    List<LectureJpaEntity> findAllByLectureStatus(LectureStatus lectureStatus);

    @Query("SELECT l FROM LectureJpaEntity l JOIN FETCH l.dayOfWeekVO d WHERE l.lectureStatus = :lectureStatus AND l.acceptableStudent.targetGrade = :targetGrade")
